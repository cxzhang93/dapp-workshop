...
contract Election {
    ...
    constructor(uint _durationInMinutes, address _admin, string _electionName) public {
        require(_durationInMinutes > 0 && _durationInMinutes <= 525600, "Invalid duration");
        admin = _admin;
    ...
    function addConsituency(uint _consituencyId, string _name) public onlyAdmin {
        require(electionStatus && now < electionDuration, "Registration closed");
        require(_consituencyId > 0, "Invalid constituency");
    ...
    function addCandidate(address _candidateId, string _name, string _email, string _phoneNo, uint _consituencyId, string _party) public onlyAdmin{
        ...
        require(consituencyExist[_consituencyId], "Unknown constituency");
    ...
    function addVoter(address _voterId, string _name, string _email,string _phoneNo, uint _consituencyId, uint8 _age)
        public onlyAdmin{
        require(electionStatus && now < electionDuration, "Registration closed");
        require(_voterId != address(0), "Invalid voter");
        require(consituencyExist[_consituencyId], "Unknown constituency");
    ...
    function castVote(uint _consituencyId, address _candidateId) public returns(bool status) {
        ...
        require(now < electionDuration, "Election deadline passed");
        require(voterExist[msg.sender], "Voter not registered");
        require(consituencyExist[_consituencyId], "Unknown constituency");
        require(candidateExist[_candidateId], "Unknown candidate");
        require(voterData[msg.sender].consituencyId == _consituencyId, "Wrong voter constituency");
        require(candidateData[_candidateId].consituencyId == _consituencyId, "Wrong candidate constituency");
